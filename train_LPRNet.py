...
def get_parser():
    ...
    parser.add_argument('--train_img_dirs', default="train_in", help='the train images path')
    parser.add_argument('--test_img_dirs', default="valid_in", help='the test images path')
    ...
    parser.add_argument('--save_interval', default=50, type=int, help='interval for save model state dict')
    parser.add_argument('--test_interval', default=50, type=int, help='interval for evaluate')
...
def Greedy_Decode_Eval(lprnet, datasets, args):
    lprnet.eval()
    epoch_size = len(os.listdir(datasets)) // args.test_batch_size
    test_dataset = LPRDataLoader(datasets, args.img_size, args.lpr_max_len)
    batch_iterator = iter(DataLoader(test_dataset, args.test_batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=collate_fn))
...
def train():
    ...
    epoch_size = len(os.listdir(train_img_dirs)) // args.train_batch_size
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if (iteration + 1) % args.test_interval == 0:
            Greedy_Decode_Eval(lprnet, test_img_dirs, args)
